...
为企业级 Agent 平台 Web 控制台增加 Workflow 工作台，用于本地自测工作流草稿、步骤状态、调度配置和 schedule run 状态。
...
当前后端尚未提供独立 `POST /workflows/{id}/run` 接口，因此 Web 首版将“运行 workflow”落到现有能力：
...
- 增加真正的 workflow run endpoint 后，页面直接调用运行接口。
